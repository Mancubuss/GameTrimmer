...
1. Перевірити (юніт-тест або ручний прогін), що `db_path()`, `ensure_rules_path()`, `ensure_l10n_rules_path()` завжди повертають шлях відносно `current_exe()`, незалежно від CWD запуску (напр. запустити exe подвійним кліком із Провідника **і** з `cmd.exe` в іншій теці — результат має бути ідентичний).
2. Зафіксувати в тестах/CI, що `Cargo.lock` не містить `directories`/`ron` (простий grep-тест або `cargo tree -i eframe` без `persistence`-фічі) — щоб майбутнє оновлення `eframe` не тихо увімкнуло запис у `%APPDATA%`.

**P1 — дрібні, безпечні, безконфліктні з паралельною роботою:**
3. `crates/core/src/db.rs`, у `configure()` (після рядка 85): додати
   `conn.pragma_update(None, "temp_store", "MEMORY")?;`
   — прибирає єдиний реальний ризик запису поза текою програми (тимчасові файли `VACUUM`/сортувань у `%TEMP%`).
4. `crates/app/src/app.rs:154` і `crates/app/src/worker/scan.rs:115`: доповнити текст помилки відкриття БД підказкою про права на запис (текст-only зміна, без зміни логіки).
5. `crates/app/build.rs` + новий `crates/app/assets/gametrimmer.manifest`: додати DPI/longPath-маніфест (розділ «Пакування» вище) — робити **після** ручної DPI-перевірки з чек-листа, щоб знати, чи це взагалі потрібно, чи просто підстраховка.
6. Створити `scripts/package-portable.ps1` за наведеним вище текстом.

**P2 — потребують ручного тестування на реальному носії, робити перед релізом:**
7. Запустити зібраний портабельний zip з реальної USB-флешки (по можливості і exFAT, і FAT32) — перевірити: створення БД, WAL-файлів, сканування, видалення (обома методами — Permanent і RecycleBin).
8. Розпакувати в `C:\Program Files\GameTrimmer` без прав адміністратора — перевірити, що з'являється зрозуміла помилка (п. 4 вище), а не тиха відмова чи краш.
9. Прогнати DPI-чек-лист (100/150/200%, per-monitor якщо є обладнання).
10. (Опційно, окремо від портабельності) Перевірити поведінку `trash::delete` для файлів на знімному носії без Кошика.

**P3 — поза межами цього аудиту, але варте окремого review (не робити зараз):**
11. `panic = "abort"` у `[profile.release]` (кореневий `Cargo.toml`) технічно вимикає ефективність `catch_unwind` у `crates/app/src/worker/scan.rs:528` та `crates/core/src/mftscan/mod.rs:151` у релізній збірці. Це питання стійкості до паніки одного провайдера/задачі, не портабельності файлової системи — фіксую як окрему знахідку, рішення (напр. `panic = "unwind"` для release, чи прибрати `catch_unwind`-залежність) вимагає окремого обговорення тредоф-у розмір/швидкість vs стійкість.
