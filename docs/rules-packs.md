...
A third built-in table, `game_reference_local.json`, is **not** a rule pack
and merges differently — see [The per-game catalogue](#the-per-game-catalogue)
below. It has an overlay of its own, installed under a different name.
...
`game_reference_local.json` at the repo root is what a catalogue names for one
specific game: today, that game's intro videos, file by file. It is embedded
the same way (`BUILTIN_GAME_REFERENCE_JSON` in `crates/core/src/reference.rs`)
and written by hand.
...
- **It is a table, not rules.** One row per store id: `app_id`, `title`, and
  ...
  lookup per file.
- **Its descriptions are generated**, from the game's title and from which
  catalogue the entry came out of, so the same sentence is not stored hundreds
  of times in two languages — and so a row never claims a source it did not
  come from.
- **Entries may carry a `note`**, which the program ignores. JSON has no
  comments, and a curated list whose entries carry no reason is a list nobody
  dares change later.
...
### Installing a bigger catalogue

The built-in catalogue is small — it holds the games this project checked
itself. A larger one is installed the same way a `rules.json` overlay is:
**put a `game_reference.json` next to `gametrimmer.exe`.** It is read on the
next scan and folded in; a game named by both keeps every file either one
names, because an entry exists in one catalogue precisely where the other
missed something.

The file is read, never written, and never created — if it is not there,
scanning runs on the built-in entries and that is the normal case, not a
degraded one. A file that does not parse is reported as a warning and skipped:
refusing to scan because an optional extra is malformed would be the worse
answer, but it is said out loud, because "the games I installed a catalogue
for are not being found" would otherwise look exactly like the file being
ignored.

Format: identical to `game_reference_local.json` in the repo root, which is
the built-in one and doubles as the worked example.
  ...
  "games": [
    {
      ...
      "note": "optional, ignored by the program",
      "intro_files": ["UnrealWise_1080p30.mp4", "ReflectorLogo_1080p30.mp4"]
    }
  ]
}
```

`app_id` is the store id a launcher records for the installed game — a Steam
appid, a GOG product id — the same value `games.app_id` holds. An entry with
no `app_id`, a duplicate `app_id`, or a non-ASCII file name is refused: the
matcher folds case the ASCII way, and an entry bound to nothing would claim
every game at once.

**GameTrimmer ships no such catalogue and names no source for one.** Where an
installed file came from, and on what terms it may be used and passed on, is
that file's own business and its author's to state.
