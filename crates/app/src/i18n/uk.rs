...
pub(super) const STRINGS: Strings = Strings {
    ...
    onboarding_how_body: "Нічого не оцінюється за розміром чи віком файла. Бібліотеки \
         перебирають три детектори: набір правил зіставляє назви файлів і тек із відомими \
         шаблонами (rules.json), мовний детектор упізнає файли локалізацій за мовними \
         маркерами (l10n_rules.json), а перевірка на осиротіле порівнює те, на що в \
         лаунчера ще є маніфест, із тим, що насправді лежить на диску. Обидва файли правил \
         лежать поруч із програмою — їх можна прочитати, змінити або замінити своїми.",
    onboarding_filters_body: "Що саме потрапляє під сканування, вирішують два фільтри — \
         обидва в Налаштуваннях \u{2192} Сканування. Категорії: осиротіле, дистрибутиви, \
         документація, бонусні матеріали, локалізації, інші рештки розробника — вимкнена \
         категорія ніколи не аналізується, не показується й не зберігається. Список мов, які \
         ніколи не позначаються: що б не знайшов детектор, мову з цього списку ніколи не \
         пропонують до видалення.",
    ...
    onboarding_review_mark: "\u{26a0} біля файла означає, що застосунок менш упевнений, чим цей \
         файл є насправді. Позначене так ніколи не обирається за вас — спершу гляньте, потім \
         вирішуйте.",
    onboarding_safety: "Сканування нічого не змінює на диску. Ніщо не видаляється, доки ви не \
         позначите це й не підтвердите.",
    onboarding_logging_body: "З увімкненим журналом програма пише gametrimmer.log поруч із \
         собою: помилки й те, що сканування робило на якому томі. Файл лишається на вашій \
         машині — нікуди нічого не надсилається, жодної телеметрії в програмі немає. Але \
         якщо ви колись натрапите на хибну знахідку чи сканування, яке впало, саме цей файл \
         відрізняє звіт, який можна полагодити, від здогадів. Увімкнути зараз нічого не \
         коштує й може позбавити потреби відтворювати проблему потім. Вимкнути можна будь-\
         коли в Налаштуваннях \u{2192} Дані й діагностика.",

    disclaimer_heading: "ВИКОРИСТАННЯ НА ВЛАСНИЙ РИЗИК",
    disclaimer_body: "GameTrimmer видаляє позначені вами файли — остаточно або в Кошик, як \
         ви оберете. Виявлення евристичне: файл можна визначити хибно, а видалення не того \
         файла здатне лишити гру нездатною запуститися. Програму надано «як є», без жодних \
         гарантій, і її автори не несуть відповідальності за втрачені дані, зламані \
         встановлення ігор чи будь-що інше, що випливає з її використання (див. LICENSE \
         поруч із програмою). Що видаляти — ваше рішення і ваша відповідальність; тримайте \
         резервні копії того, чого не можна завантажити наново. Самі ігрові файли зазвичай \
         відновлюються перевіркою цілісності або перевстановленням із магазину — саме на цю \
