...
pub(super) const STRINGS: Strings = Strings {
    ...
    onboarding_step_scan: "1. Просканувати. GameTrimmer читає ваші бібліотеки ігор і показує, \
         що можна прибрати: локалізації, якими ви не користуєтесь, рештки видалених ігор, \
         рантайми, бонусні матеріали.",
    onboarding_step_review: "2. Переглянути. Знахідки згруповані за диском і грою. Нічого не \
         приховано — кожен файл видно, разом із повним шляхом.",
    onboarding_step_remove: "3. Видалити позначене. Спосіб обираєте ви: остаточно або в Кошик.",
    onboarding_profile: "Профіль — це те, наскільки сміливо застосунок позначає файли за вас. \
         \u{ab}Обережний\u{bb} позначає лише те, чого лаунчер сам не поверне; \
         \u{ab}Агресивний\u{bb} — більше. Позначати й знімати позначки руками можна завжди.",
    onboarding_confidence: "Довіра — це те, наскільки застосунок упевнений, що файл є тим, чим \
         здається. Усе нижче за 80% позначено знаком попередження, і на це варто глянути перед \
         видаленням.",
