...
pub(super) const STRINGS: Strings = Strings {
    ...
    rules_pack_absent_label: "Not present - built-in rules in use",
    ...
    rules_hint: "Both files are optional and GameTrimmer never writes them. It analyses with its own built-in rules, so updating the program updates them. Put a file of either name in the folder above and it is applied on top of the built-in rules from the next scan; remove it and the built-ins stand alone. Format and templates: docs/rules-packs.md.",
