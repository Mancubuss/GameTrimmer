...
task_ids = [360, 361, 362, 363, 364, 365, 366, 367, 368]

for tid in task_ids:
    url = f"http://127.0.0.1:3456/api/v1/tasks/{tid}"
    payload = json.dumps({"done": True}).encode('utf-8')
    req = urllib.request.Request(url, data=payload, headers=headers, method="POST")
    try:
        with urllib.request.urlopen(req) as resp:
            data = json.loads(resp.read().decode('utf-8'))
            print(f"Task {tid} ({data.get('title', '')}) marked as DONE (done={data.get('done')})")
    except Exception as e:
        print(f"Error updating task {tid}: {e}")

print("All GT-EP16 tasks marked as DONE on Vikunja board.")
